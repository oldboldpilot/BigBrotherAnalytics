...
- **ML Model:** 60-feature neural network [256→128→64→32 architecture], 56.3% 5-day, 56.6% 20-day accuracy
...
- **Valgrind v3.24.0** - Memory leak detection & thread safety validation (built from source)
  - Zero leaks detected across all C++ modules
...
9. **Memory Safety:** All C++ code must pass Valgrind with zero leaks (run `./benchmarks/run_valgrind_tests.sh`)
...
- ✅ Schwab API responses (quotes, options, accounts) - 120+ req/min
- ✅ NewsAPI feeds (large arrays) - 96 req/day
- ✅ Any JSON parsing in hot paths (>10 req/min)
- ❌ Configuration files (use nlohmann for convenience)
- ❌ OAuth token responses (low frequency)
- ❌ Test fixtures (use nlohmann for readability)
...
- NewsAPI: **23.0x faster** (8474ns → 369ns)
...
│   ├── employment_signals.cppm       # Employment signal generation
│   ├── market_intelligence.cppm      # Main intelligence module
│   ├── price_predictor.cppm          # ML price predictor (ONNX Runtime, 525 lines)
│   └── feature_extractor.cppm        # 60-feature extraction with AVX2 SIMD (620 lines)
├── trading_decision/
...
ai/
├── CLAUDE.md                         # AI assistant guide with ML v3.0 details
├── MODEL_EVOLUTION_SUMMARY.md        # ML model version history
└── CUSTOM_MODEL_SUMMARY.md           # v3.0 model technical details

models/
├── price_predictor.onnx              # ONNX model (60 features)
├── price_predictor_info.json         # Model metadata (accuracy, features)
└── price_predictor_best.pth          # PyTorch checkpoint
```

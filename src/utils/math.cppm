/**
 * BigBrotherAnalytics - Math Module (C++23)
 *
 * Statistical and financial mathematics using modern C++23 features.
 *
 * Features:
 * - Statistical functions (mean, variance, correlation, etc.)
 * - Financial math (returns, Sharpe ratio, drawdown)
 * - C++23 ranges and views for efficient computation
 ...
 * - T.10: Specify concepts for template arguments
 *
...
/**
 * Numeric concept - any arithmetic type
...
/**
 * FloatingPoint concept - floating-point types only
 */
template<typename T>
 ...
 * F.6: noexcept - no exceptions thrown
...
[[nodiscard]] auto percentile(R&& range, double p) -> double {
    ...
    // Copy and sort (COW optimization)
 ...
 * @return Vector of rolling means
 */
template<std::ranges::range R>
    requires Numeric<std::ranges::range_value_t<R>>
 ...
 * @return Maximum drawdown (0.0 to 1.0)
 */
template<std::ranges::range R>
    requires FloatingPoint<std::ranges::range_value_t<R>>

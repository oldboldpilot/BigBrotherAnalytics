...
class StressTestingEngine {
  public:
    ...
    [[nodiscard]] auto runAllScenarios() const noexcept -> std::vector<StressTestResult> {

        std::vector<StressTestResult> results;

        auto scenarios = {StressScenario::MarketCrash,    StressScenario::VolatilitySpike,
                          StressScenario::SectorRotation, StressScenario::InterestRateShock,
                          StressScenario::BlackSwan,      StressScenario::FlashCrash};
  ...
  private:
    ...
    StressTestingEngine(StressTestingEngine&& other) noexcept
        : positions_(std::move(other.positions_)) {
        // mutex_ is default-constructed
    }

    // Move assignment - mutex cannot be moved
    StressTestingEngine& operator=(StressTestingEngine&& other) noexcept {
        if (this != &other) {
            positions_ = std::move(other.positions_);
    ...
    StressTestingEngine(StressTestingEngine const&) = delete;
    StressTestingEngine& operator=(StressTestingEngine const&) = delete;
    ...
    [[nodiscard]] auto calculatePortfolioValue() const noexcept -> double {
        ...
        for (size_t i = 0; i < vec_size; i += 4) {
            __m256d qty = _mm256_set_pd(positions_[i + 3].quantity, positions_[i + 2].quantity,
                                        positions_[i + 1].quantity, positions_[i].quantity);
            __m256d price =
                _mm256_set_pd(positions_[i + 3].current_price, positions_[i + 2].current_price,
                              positions_[i + 1].current_price, positions_[i].current_price);
    ...
    [[nodiscard]] auto stressMarketCrash() const noexcept -> Result<StressTestResult> {
        ...
        for (size_t i = 0; i < vec_size; i += 4) {
            ...
            __m256d price =
                _mm256_set_pd(positions_[i + 3].current_price, positions_[i + 2].current_price,
                              positions_[i + 1].current_price, positions_[i].current_price);
            __m256d beta = _mm256_set_pd(positions_[i + 3].beta, positions_[i + 2].beta,
                                         positions_[i + 1].beta, positions_[i].beta);
            ...
            for (int j = 0; j < 4; ++j) {
                double pnl = positions_[i + j].quantity *
                             (stressed_prices[j] - positions_[i + j].current_price);
    ...
    [[nodiscard]] auto stressVolatilitySpike() const noexcept -> Result<StressTestResult> {
        ...
        Logger::getInstance().info("Volatility Spike Stress Test: P&L ${:.2f} ({:.1f}%)",
    ...
    [[nodiscard]] auto stressSectorRotation() const noexcept -> Result<StressTestResult> {
        ...
        Logger::getInstance().info("Sector Rotation Stress Test: P&L ${:.2f} ({:.1f}%)", result.pnl,
                                   result.pnl_percentage * 100);
    ...
    [[nodiscard]] auto stressInterestRateShock() const noexcept -> Result<StressTestResult> {
        ...
        Logger::getInstance().info("Interest Rate Shock Stress Test: P&L ${:.2f} ({:.1f}%)",
                                   result.pnl, result.pnl_percentage * 100);
    ...
    [[nodiscard]] auto stressCreditCrunch() const noexcept -> Result<StressTestResult> {
        ...
        Logger::getInstance().info("Credit Crunch Stress Test: P&L ${:.2f} ({:.1f}%)", result.pnl,
                                   result.pnl_percentage * 100);
    ...
    [[nodiscard]] auto stressBlackSwan() const noexcept -> Result<StressTestResult> {
        ...
        Logger::getInstance().warn("Black Swan Stress Test: CRITICAL - P&L ${:.2f} ({:.1f}%)",
                                   result.pnl, result.pnl_percentage * 100);
    ...
    [[nodiscard]] auto stressFlashCrash() const noexcept -> Result<StressTestResult> {
        ...
        Logger::getInstance().info("Flash Crash Stress Test: P&L ${:.2f} ({:.1f}%)", result.pnl,
                                   result.pnl_percentage * 100);
